...
const SearchTab: React.FC<SearchProps> = ({ route }) => {
  ...
  return (
    <ScrollView className="bg-gray-100">
      ...
      {/* Search Result */}
      <View className="mx-5 mt-6">
        <Text className="text-center text-3xl font-bold text-gray-800">Kết quả tìm kiếm cho:</Text>
        <Text className="mt-1 text-center text-2xl font-bold text-orange-500">
          {query || 'Không có'}
        </Text>
      </View>
      ...
      <View className="mx-5">
        {query ? (
          <>
            ...
            <Text className="mt-2 text-center text-lg text-gray-500">Bạn có thể thử:</Text>
            <View className="mt-6">
              {suggestedFoods.map((item) => (
                <View key={item.id}>{renderSuggestedFood({ item })}</View>
              ))}
            </View>
          </>
        ) : (
