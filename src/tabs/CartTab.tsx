...
const CartTab: React.FC = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text className="text-center text-3xl font-bold">GIỎ HÀNG CỦA BẠN</Text>
      <FlatList
        data={cartItems}
        keyExtractor={(item) => item.id.toString()}
        renderItem={({ item }) => (
          <View className="flex flex-row items-center justify-between border-b border-gray-300 p-4">
            {/* Hình ảnh sản phẩm */}
            <Image source={item.image} style={styles.image} />
            ...
            <View className="mx-4 flex-1">
              <Text
                className="overflow-hidden text-ellipsis text-2xl font-semibold"
                numberOfLines={2}>
                {item.name}
              </Text>
              <Text className="mt-1 text-xl font-semibold text-orange-500">{item.price}₫</Text>
              <Text className="text-xl font-semibold text-orange-500">
                Số lượng: {item.quantity}
              </Text>

              {/* Nút tăng/giảm số lượng */}
              <View className="fflex mt-2 flex-row items-center justify-center space-x-4">
                <TouchableOpacity
                  className="mx-4 rounded-full border border-gray-400 p-3"
                  onPress={() => removeFromCart?.(item.id)}>
                  <Text className="text-2xl font-bold text-gray-700">-</Text>
                </TouchableOpacity>

                <Text className="mx-6 text-center text-xl font-semibold">{item.quantity}</Text>

                <TouchableOpacity
                  className="mx-4 rounded-full bg-orange-500 p-3"
      ...
      <Text style={styles.total}>Tổng: {calculateTotal()}₫</Text>
      <View className="mx-4 flex flex-row items-center justify-between px-4">
        <Button title="Làm trống" onPress={clearCart} color="#FF4D4D" />
        <Button title="Thanh toán" onPress={handleCheckout} color="#4CAF50" />
      </View>
    </View>
  );
};
